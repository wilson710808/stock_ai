...
1. 每個章節用 ## 標題
2. **所有分析內容必須使用列表（bullet points）呈現**，禁止使用 Markdown 表格
3. 數據和指標用列表格式：- **指標名稱**：數值 | 評價
4. 重要結論用 **粗體**
5. 優缺點用 ✅ ❌ 標注
5. 分隔線用 ---
7. **佔比分析時，請提供圓餅圖數據**（用以下格式標記，前端會自動渲染）：
   \`\`\`pie
   {"labels":["項目A","項目B"],"values":[60,40],"colors":["#22c55e","#3b82f6"]}
   \`\`\`
8. 免責聲明放最後
...
### 推薦輸出規範（非常重要）
當分析 prompt 要求輸出 [RECOMMENDATION:XXX] 標記時，你**必須**在回覆最後一行單獨輸出該標記，不可省略。此標記用於系統解析，不會顯示給用戶。

**免責聲明：** 本分析僅供參考，不構成投資建議。`;


// 構建用戶投資上下文（持倉 + 自選股）→ 注入 AI system prompt
...
> 如果建議買入，請提供合適的目標買入價及目標賣出價
> 如果建議觀望持有，請提供合適的賣出價位
> 如果建議避開，請說明是否有合適的買入價位還是堅決不碰
...
### 🏢 公司對比
- **市值**：${ticker} $XXX | 對手A $XXX | 對手B $XXX
- **營收**：${ticker} $XXX | 對手A $XXX | 對手B $XXX
- **市佔率**：${ticker} XX% | 對手A XX% | 對手B XX%
- **P/E**：${ticker} XX | 對手A XX | 對手B XX
